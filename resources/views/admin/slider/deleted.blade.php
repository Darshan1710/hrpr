...
@section('page-content')
    <div class="main-container container-fluid">
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col">
                    <h1 class="text-primary mr-auto">Inactive Slider</h1>
                </div>
            </div>
        </div>
        <!-- /heading -->
                ...
                @if ($sliderList->isNotEmpty())
                    @foreach ($sliderList as $item)
                        <tr>
                            ...
                            <td class="align-middle">{{ $item->slider_title }}</td>
                            <td class="align-middle">
                              <a href="https://www.youtube.com/embed/{{ $item->video_url }}" target="_blank">{{ $item->video_url }}</a>
                            </td>
                            <td class="align-middle">
                                <form method="POST" id="restore-data-form"
                                    action="{{ route('admin.slider.restore', $item->id) }}" hidden>
                                    {{ csrf_field() }}
                                    {{ method_field('PUT') }}
                                </form>
